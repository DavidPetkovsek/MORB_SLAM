...
void KeyFrameDatabase::DetectCandidates(std::shared_ptr<KeyFrame> pKF, float minScore,
  ...
  if (!lKFsSharingWordsLoop.empty()) {
    ...
    for (std::list<std::shared_ptr<KeyFrame>>::iterator lit = lKFsSharingWordsLoop.begin(),
  ...
  if (!lKFsSharingWordsMerge.empty()) {
    ...
    // Only compare against those keyframes that share enough words
    int maxCommonWords = 0;
    for (std::list<std::shared_ptr<KeyFrame>>::iterator lit = lKFsSharingWordsMerge.begin(),
...
void KeyFrameDatabase::DetectBestCandidates(std::shared_ptr<KeyFrame> pKF,
                                            std::vector<std::shared_ptr<KeyFrame>>& vpLoopCand,
                                            std::vector<std::shared_ptr<KeyFrame>>& vpMergeCand,
                                            int nMinWords) {
...
void KeyFrameDatabase::DetectNBestCandidates(std::shared_ptr<KeyFrame> pKF,
                                             std::vector<std::shared_ptr<KeyFrame>>& vpLoopCand,
                                             std::vector<std::shared_ptr<KeyFrame>>& vpMergeCand,
                                             int nNumCandidates) {
  std::list<std::shared_ptr<KeyFrame>> lKFsSharingWords;
