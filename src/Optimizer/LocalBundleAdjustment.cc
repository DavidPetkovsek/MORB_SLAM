...
void Optimizer::LocalBundleAdjustment(std::shared_ptr<KeyFrame> pKF, bool* pbStopFlag, std::shared_ptr<Map> pMap, bool bOdom) {
  ...
  for (std::list<std::shared_ptr<KeyFrame>>::iterator lit = lLocalKeyFrames.begin(), lend = lLocalKeyFrames.end(); lit != lend; lit++) {
  ...
  for (std::list<std::shared_ptr<KeyFrame>>::iterator lit = lFixedCameras.begin(), lend = lFixedCameras.end(); lit != lend; lit++) {
    std::shared_ptr<KeyFrame> pKFi = *lit;
    g2o::VertexSE3Expmap* vSE3 = new g2o::VertexSE3Expmap();
    Sophus::SE3<float> Tcw = pKFi->GetPose();
    vSE3->setEstimate(g2o::SE3Quat(Tcw.unit_quaternion().cast<double>(), Tcw.translation().cast<double>()));
    ...
    for (std::map<std::weak_ptr<KeyFrame>, std::tuple<int, int>, std::owner_less<>>::const_iterator mit = observations.begin(), mend = observations.end(); mit != mend; mit++) {
      if(std::shared_ptr<KeyFrame> pKFi = (mit->first).lock()) {
        if (!pKFi->isBad() && pKFi->GetMap() == pCurrentMap) {
          ...
          // Stereo observation
          } else if (leftIndex != -1 && pKFi->mvuRight[std::get<0>(mit->second)] >= 0) {
            const cv::KeyPoint& kpUn = pKFi->mvKeysUn[leftIndex];
  ...
  // Recover optimized data KeyFrames
  for (std::list<std::shared_ptr<KeyFrame>>::iterator lit = lLocalKeyFrames.begin(), lend = lLocalKeyFrames.end(); lit != lend; lit++) {
    std::shared_ptr<KeyFrame> pKFi = *lit;
...
void Optimizer::LocalBundleAdjustment(std::shared_ptr<KeyFrame> pMainKF, std::vector<std::shared_ptr<KeyFrame>> vpAdjustKF, std::vector<std::shared_ptr<KeyFrame>> vpFixedKF, bool* pbStopFlag) {
